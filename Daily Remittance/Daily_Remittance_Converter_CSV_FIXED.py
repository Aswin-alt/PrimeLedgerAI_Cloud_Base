...
def load_mapping(mapping_path: str | None):
    ...
    for row in workbook["Location Mapping"].iter_rows(min_row=2, values_only=True):
        ...
        # Unit 247401 must use the exact QuickBooks location name below.
        # This also corrects older mapping workbooks that may still say Marietta.
        if unit == "247401":
            locations[unit] = "7401-Youngstown"
...
def write_csv(rows, output_path: str):
    """Write QuickBooks journal rows as CSV."""
    with open(output_path, "w", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in rows:
            ...
            if hasattr(formatted[0], "strftime"):
                formatted[0] = formatted[0].strftime("%m/%d/%Y")
            formatted[3] = "" if formatted[3] is None else f"{float(formatted[3]):.2f}"
            formatted[4] = "" if formatted[4] is None else f"{float(formatted[4]):.2f}"
            formatted[6] = "" if formatted[6] is None else formatted[6]
...
class RemittanceApp:
    ...
    def convert(self):
        ...
        suggested = f"{Path(input_path).stem}_QuickBooks_Journal.csv"
